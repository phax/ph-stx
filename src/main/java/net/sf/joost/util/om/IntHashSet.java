/*
 * $Id: IntHashSet.java,v 1.1 2007/06/04 19:57:35 obecker Exp $
 *
...
/**
 * Set of int values. This class is modelled on the java.net.Set interface, but
 * it does not implement this interface, because the set members are int's
 * rather than Objects.
 * <p/>
 * Not thread safe.
 *
...
public class IntHashSet implements IntSet, Serializable
{
   ...
   */
  public IntHashSet (final int capacity, final int noDataValue)
  {
    ndv = noDataValue;
    // _factor = 0.25;
  ...
  public int getFirst (final int defaultValue)
  {
    for (final int _value : _values)
    {
      if (_value != ndv)
      {
        return _value;
  ...
  // private double _factor; // 0.0 <= _factor <= 1.0 - changed by MHK to assume
  // factor = 0.25
  ...
  /**
   * Gets the index of the value, if it exists, or the index at which this value
   * would be added if it does not exist yet.
   */
...
//
// The Initial Developer of the Original Code is Dominique Devienne of Landmark
// Graphics.
//
