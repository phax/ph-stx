/*
 * $Id: FastStringBuffer.java,v 1.1 2007/06/04 19:57:35 obecker Exp $
 *
 ...
 * StringBuffer it is not synchronized. It also offers the capability to remove
 * unused space. (This class could possibly be replaced by StringBuilder in JDK
 * 1.5, but using our own class gives more control.)
 */
...
public final class FastStringBuffer implements CharSequence, Serializable
{
  ...
  public FastStringBuffer (final CharSequence cs)
  {
  ...
  public void append (final StringBuffer s)
  {
    final int len = s.length ();
    ensureCapacity (len);
    s.getChars (0, len, array, used);
  ...
  {
    // Although we provide variants of this method for different subtypes, Java
    // decides which to use based
    // on the static type of the operand. We want to use the right method based
    // on the dynamic type, to avoid
    // creating objects and copying strings unnecessarily. So we do a dynamic
    // dispatch.
    final int len = s.length ();
    ensureCapacity (len);
    if (s instanceof CharSlice)
    {
      ...
      if (s instanceof String)
      {
        ...
        if (s instanceof FastStringBuffer)
        {
   ...
   * @param start
   *        the offset of the first character in the array to be copied
   * @param length
   ...
   * 16-bit <code>char</code>s in the sequence.
   * </p>
   *
   ...
   * from zero to <tt>length() - 1</tt>. The first <code>char</code> value of
   ...
   * sequence is <tt>end - start</tt>, so if <tt>start == end</tt> then an empty
   ...
   * @throws IndexOutOfBoundsException
   *         if <tt>start</tt> or <tt>end</tt> are negative, if <tt>end</tt> is
   *         greater than <tt>length()</tt>, or if <tt>start</tt> is greater
   *         than <tt>end</tt>
   */
   ...
   * The characters are copied into the subarray of <code>dst</code> starting at
   * index <code>dstBegin</code> and ending at index:
   * <p>
   * <blockquote>
   * 
   * <pre>
  ...
  /**
   * Get a char[] array containing the characters. The caller should not modify
   * the array.
   */
   ...
   * @param ch
   *        the new character to overwrite the existing character at that
   *        location
   * @throws IndexOutOfBoundsException
   ...
   * If this is less than the current length, characters beyond the specified
   * point are deleted.
   * 
   * @param length
   *        the new length
  ...
  /**
   * Expand the character array if necessary to ensure capacity for appended
   * data
   */
  ...
  /**
   * Remove surplus space from the array. This doesn't reduce the array to the
   * minimum possible size; it only reclaims space if it seems worth doing.
   * Specifically, it contracts the array if the amount of wasted space is more
   * than 256 characters, or more than half the allocated size.
   */
