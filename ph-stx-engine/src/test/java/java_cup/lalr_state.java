...
public class lalr_state
{
  ...
  protected static void propagate_all_lookaheads () throws internal_error
  {
    ...
    for (final Enumeration st = all (); st.hasMoreElements ();)
    {
   ...
   */
  public void add_transition (final symbol on_sym, final lalr_state to_st) throws internal_error
  {
  ...
  public static lalr_state build_machine (final production start_prod) throws internal_error
  {
  ...
  protected void propagate_lookaheads () throws internal_error
  {
    ...
    for (final Enumeration itm = items ().all (); itm.hasMoreElements ();)
   ...
   */
  public void build_table_entries (final parse_action_table act_table,
                                   final parse_reduce_table reduce_table) throws internal_error
  {
    ...
    for (final Enumeration i = items ().all (); i.hasMoreElements ();)
    {
      itm = (lalr_item) i.nextElement ();
    ...
    for (lalr_transition trans = transitions (); trans != null; trans = trans.next ())
    {
      ...
      if (!sym.is_non_term ())
      {
        ...
        {
          /* we now have at least one conflict */
          final production p = ((reduce_action) our_act_row.under_term[sym.index ()]).reduce_with ();
  ...
  protected boolean fix_with_precedence (final production p,
      ...
      /* if terminal precedes rule, put shift in table */
        ...
        { /* they are == precedence */
          ...
          /* if it is left, put reduce in table */
            ...
            if (term.precedence_side () == assoc.left)
            {
              table_row.under_term[term_index] = insert_reduce (table_row.under_term[term_index], act);
              return true;
            }
             ...
             */
              ...
              if (term.precedence_side () == assoc.nonassoc)
              {
   ...
   */
  protected parse_action insert_action (final parse_action a1,
  ...
  protected parse_action insert_shift (final parse_action a1, final parse_action a2) throws internal_error
  {
    return insert_action (a1, a2, parse_action.SHIFT);
  }

  /* find the reduce in the two actions */
  protected parse_action insert_reduce (final parse_action a1, final parse_action a2) throws internal_error
  {
  ...
  protected void report_conflicts (final terminal_set conflict_set) throws internal_error
  {
    ...
    for (final Enumeration itms = items ().all (); itms.hasMoreElements ();)
    {
      ...
      if (itm.dot_at_end ())
      {
        ...
        for (final Enumeration comps = items ().all (); comps.hasMoreElements ();)
        {
        ...
        final terminal_set lookahead = itm.lookahead ();
        for (int t = 0; t < terminal.number (); t++)
   ...
   */
  protected void report_reduce_reduce (final lalr_item itm1, final lalr_item itm2) throws internal_error
  {
   ...
   */
  protected void report_shift_reduce (final lalr_item red_itm, final int conflict_sym) throws internal_error
  {
    lalr_item itm;
    symbol shift_sym;
