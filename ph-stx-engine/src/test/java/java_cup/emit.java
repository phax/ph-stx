...
public class emit
{
  ...
  protected static void set_locations (final boolean b)
  {
    _locations = b;
  }

  protected static void set_genericlabels (final boolean b)
  {
    _genericlabels = b;
  }

  protected static void set_xmlactions (final boolean b)
  {
    _xmlactions = b;
    if (!b)
      return;
   ...
   */
  public static void symbols (final PrintWriter out, final boolean emit_non_terms, final boolean sym_interface)
  {
    terminal term;
    non_terminal nt;
   ...
   *        the start production of the grammar.
   */
  protected static void emit_action_code (final PrintWriter out, final production start_prod) throws internal_error
  {
    production prod;
    ...
    for (int instancecounter = 0; instancecounter <= production.number () / UPPERLIMIT; instancecounter++)
    {
                                       ...
                                       production.number ()); prod = production.find (++proditeration))
      {
        ...
        {
          final int lastResult = ((action_production) prod).getIndexOfIntermediateResult ();
          if (lastResult != -1)
          {
            result = "(" +
                     ...
                     // TUM 20050917
                     ((lastResult == 1) ? ".peek()"
                                        : (".elementAt(" + emit.pre ("top") + "-" + (lastResult - 1) + ")")) +
                     ").value";
          }
        }
        ...
        /*
         * here we have the left and right values being propagated. must make
         * this a command line option. frankf 6/18/96
         */
        ...
        if (emit.lr_values ())
        {
          ...
          // TUM 20050917
          // int roffset = 0;
          rightstring = "((java_cup.runtime.Symbol)" +
                        emit.pre ("stack") +
                        // TUM 20050917
    ...
    if (production.number () < UPPERLIMIT)
    { // Make it simple for the optimizer to inline!
   ...
   */
  protected static void do_action_table (final PrintWriter out,
  ...
  protected static void do_table_as_string (final PrintWriter out, final short [] [] sa)
  {
  ...
  protected static int do_newline (final PrintWriter out, final int nchar, final int nbytes)
  {
    if (nbytes > 65500)
    {
      out.println ("\", ");
   ...
   */
  public static void parser (final PrintWriter out,
   ...
   *        the start production of the grammar.
   */
  protected static void emit_xmlaction_code (final PrintWriter out, final production start_prod) throws internal_error
  {
    production prod;
    ...
    for (int instancecounter = 0; instancecounter <= production.number () / UPPERLIMIT; instancecounter++)
    {
                                       ...
                                       production.number ()); prod = production.find (++proditeration))
      {
        ...
        for (int rhsi = 0; rhsi < prod.rhs_length (); rhsi++)
        {
          ...
          final symbol_part sym = (symbol_part) prod.rhs (rhsi);
          if (label == null)
          {
            if (!_genericlabels)
              continue;
          ...
          if (sym.the_symbol ().is_non_term ())
            ...
            nested += ",new XMLElement.Terminal(" +
                      label +
                      "xleft,\"" +
                      label +
                      "\"," +
        ...
        final String lhsname = prod.lhs ().the_symbol ().name ().replace ('$', '_');
