...
/**
 * A set of NFA states (= integers). Very similar to java.util.BitSet, but is
 * faster and doesn't crash
 *
 * @author Gerwin Klein
 * @version JFlex 1.6.1
 */
public final class StateSet
{
  ...
  public StateSet (final int size)
  {
    bits = new long [size2nbits (size)];
  }

  public StateSet (final int size, final int state)
  {
    this (size);
    addState (state);
  }

  public StateSet (final StateSet set)
  {
  ...
  private int size2nbits (final int size)
  {
    return ((size >> BITS) + 1);
  }
  ...
  {
    final int needed = size2nbits (size);

    // if (needed < bits.length) return;

    final long newbits[] = new long [Math.max (bits.length * 4, needed)];
  ...
  public boolean isElement (final int state)
  {
    final int index = state >> BITS;
    if (index >= bits.length)
      return false;
  ...
  /**
   * Returns one element of the set and removes it. Precondition: the set is not
   * empty.
   */
  ...
  public void remove (final int state)
  {
    final int index = state >> BITS;
    if (index >= bits.length)
      return;
  ...
  /**
   * Returns the set of elements that contained are in the specified set but are
   * not contained in this set.
   */
  public StateSet complement (final StateSet set)
  {
    ...
    int i;
    final int m = Math.min (bits.length, set.bits.length);
  ...
  public void add (final StateSet set)
  {
    ...
    long tbits[];
    final long sbits[] = set.bits;
    final int sbitsl = sbits.length;
  ...
  public boolean containsSet (final StateSet set)
  {
    ...
    int i;
    final int min = Math.min (bits.length, set.bits.length);
   ...
   */
  public void copy (final StateSet set)
  {
  ...
  {
    final StateSetEnumerator set = states ();
