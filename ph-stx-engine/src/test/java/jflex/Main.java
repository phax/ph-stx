...
public class Main
{
  ...
  /** JFlex version */
  public final static String version = "1.6.1"; //$NON-NLS-1$
   ...
   */
  public static void generate (final File inputFile)
  {

    Out.resetCounters ();

    final Timer totalTime = new Timer ();
    ...
    try
    {
      ...
      final Emitter e = new Emitter (inputFile, parser, dfa);
      e.emit ();
  ...
  public static List <File> parseOptions (final String argv[]) throws SilentExit
  {
    ...
    for (int i = 0; i < argv.length; i++)
    {
      ...
      // TODO: In the JFlex version after 1.6, --inputstreamctor will be
      // removed.
      ...
      // TODO: In the JFlex version after 1.6, --noinputstreamctor will be
      // removed.
      ...
      if (argv[i].equals ("--uniprops") || argv[i].equals ("-uniprops")) //$NON-NLS-1$ //$NON-NLS-2$
      {
        ...
        final String unicodeVersion = argv[i];
        try
        {
          printUnicodePropertyValuesAndAliases (unicodeVersion);
        }
        catch (final UnicodeProperties.UnsupportedUnicodeVersionException e)
        {
  ...
  /**
   * Prints one Unicode property value per line, along with its aliases, if any,
   * for the given unicodeVersion.
   *
   * @param unicodeVersion
   *        The Unicode version to print property values and aliases for
   * @throws UnicodeProperties.UnsupportedUnicodeVersionException
   *         if unicodeVersion is not supported
   */
  private static void printUnicodePropertyValuesAndAliases (final String unicodeVersion) throws UnicodeProperties.UnsupportedUnicodeVersionException
  {
    final Pattern versionPattern = Pattern.compile ("(\\d+)(?:\\.(\\d+))?(?:\\.\\d+)?");
    final Matcher matcher = versionPattern.matcher (unicodeVersion);
    if (!matcher.matches ())
    {
      throw new UnicodeProperties.UnsupportedUnicodeVersionException ();
    }
    final String underscoreVersion = matcher.group (1) + (null == matcher.group (2) ? "_0" : "_" + matcher.group (2));

    String [] propertyValues;
    String [] propertyValueAliases;
    try
    {
      final Class <?> clazz = Class.forName ("jflex.unicode.data.Unicode_" + underscoreVersion);
    ...
    final SortedMap <String, SortedSet <String>> propertyValuesToAliases = new TreeMap<> ();
    ...
    {
      final String alias = propertyValueAliases[i];
      final String value = propertyValueAliases[i + 1];
    ...
    for (final Map.Entry <String, SortedSet <String>> entry : propertyValuesToAliases.entrySet ())
    {
  ...
  public static void generate (final String argv[]) throws SilentExit
  {
    final List <File> files = parseOptions (argv);
   ...
   * up a window to choose a file, when <code>argv</code> doesn't have any file
   * entries.
   *
   * @param argv
   *        the commandline.
