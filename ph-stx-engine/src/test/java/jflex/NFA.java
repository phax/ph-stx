...
/**
 * NFA representation in JFlex. Contains algorithms RegExp -> NFA and NFA ->
 * DFA.
 *
 * @author Gerwin Klein
 * @version JFlex 1.6.1
...
public final class NFA
{
  ...
  /**
   * epsilon[current_state] is the set of states that can be reached from
   * current_state via epsilon edges
   */
  StateSet [] epsilon;
  ...
  /**
   * action[current_state]: the action associated with the state current_state
   * (null, if there is no action for the state)
   */
  Action [] action;
  ...
  public NFA (final int numInput, final int estSize)
  {
  ...
  /**
   * Construct new NFA. Assumes that lookahead cases and numbers are already
   * resolved in RegExps.
   * 
   * @see RegExps#checkLookAheads()
   */
  public NFA (final int numInput,
              final LexScan scanner,
              final RegExps regExps,
              final Macros macros,
              final CharClasses classes)
  {
    ...
    final int new_num = numEntryStates ();
  ...
  /**
   * Add a standalone rule that has minimum priority, fires a transition on all
   * single input characters and has a "print yytext" action.
   */
  public void addStandaloneRule ()
  {
    final int start = numStates;
    final int end = numStates + 1;
   ...
   */
  public void addRegExp (final int regExpNum)
  {

    if (Options.DEBUG)
      Out.debug ("Adding nfa for regexp " + regExpNum + " :" + Out.NL + regExps.getRegExp (regExpNum));

    final IntPair nfa = insertNFA (regExps.getRegExp (regExpNum));
    ...
    for (final Integer stateNum : lexStates)
    {
    ...
    {
      final Action a = regExps.getAction (regExpNum);
      ...
      {
        final RegExp r1 = regExps.getRegExp (regExpNum);
        final RegExp r2 = regExps.getLookAhead (regExpNum);

        final IntPair look = insertNFA (r2);
        ...
        {
          // base forward pass
          final IntPair forward = insertNFA (r1);
          // lookahead backward pass
          final IntPair backward = insertNFA (r2.rev (macros));
          ...
          final int entry = 2 * (regExps.getLookEntry (regExpNum) + numLexStates);
   ...
   *
   * @param lookAhead
   *        a lookahead of which isFiniteChoice is true
   * @param baseEnd
   *        the end state of the base expression NFA
   * @param a
   *        the action of the expression
   * @see SemCheck#isFiniteChoice(RegExp)
   */
  private void insertLookAheadChoices (final int baseEnd, final Action a, final RegExp lookAhead)
  {
    if (lookAhead.type == sym.BAR)
    {
      final RegExp2 r = (RegExp2) lookAhead;
      insertLookAheadChoices (baseEnd, a, r.r1);
      insertLookAheadChoices (baseEnd, a, r.r2);
    }
    else
      if (lookAhead.type == sym.MACROUSE)
      {
        final RegExp1 r = (RegExp1) lookAhead;
        insertLookAheadChoices (baseEnd, a, macros.getDefinition ((String) r.content));
      }
      else
      {
        final int len = SemCheck.length (lookAhead);

        if (len >= 0)
        {
          // termination case
          final IntPair look = insertNFA (lookAhead);

          addEpsilonTransition (baseEnd, look.start);

          final Action x = a.copyChoice (len);
          action[look.end] = x;
          isFinal[look.end] = true;
        ...
        {
          // should never happen
          throw new Error ("When inserting lookahead expression: unkown expression type " + //$NON-NLS-1$
                           lookAhead.type +
                           " in " + //$NON-NLS-1$
                           lookAhead);
        }
   ...
   */
  private void ensureCapacity (final int newNumStates)
  {
    final int oldLength = epsilon.length;

    if (newNumStates < oldLength)
      return;

    final int newStatesLength = Math.max (oldLength * 2, newNumStates);

    final boolean [] newFinal = new boolean [newStatesLength];
    final boolean [] newIsPush = new boolean [newStatesLength];
    final Action [] newAction = new Action [newStatesLength];
    final StateSet [] [] newTable = new StateSet [newStatesLength] [numInput];
    final StateSet [] newEpsilon = new StateSet [newStatesLength];
  ...
  public void addTransition (final int start, final int input, final int dest)
  {
    Out.debug ("Adding transition (" + start + ", " + input + ", " + dest + ")");

    final int maxS = Math.max (start, dest) + 1;
  ...
  public void addEpsilonTransition (final int start, final int dest)
  {
    final int max = Math.max (start, dest) + 1;
    ensureCapacity (max);
    if (max > numStates)
      numStates = max;
  ...
  /**
   * Returns <code>true</code>, iff the specified set of states contains a final
   * state.
   *
   * @param set
   *        the set of states that is tested for final states.
   */
  private boolean containsFinal (final StateSet set)
  {
    states.reset (set);
   ...
   *
   * @param set
   *        the set of states that is tested for pushback-states. private
   *        boolean containsPushback(StateSet set) { states.reset(set); while (
   *        states.hasMoreElements() ) if ( isPushback[states.nextElement()] )
   ...
   */
  private Action getAction (final StateSet set)
  {
    ...
    while (states.hasMoreElements ())
    {
      ...
      final Action currentAction = action[states.nextElement ()];
  ...
  /**
   * Calculates the epsilon closure for a specified set of states. The epsilon
   * closure for set a is the set of states that can be reached by epsilon edges
   * from a.
   *
   * @param startState
   *        the start state for the set of states to calculate the epsilon
   *        closure for
   * @return the epsilon closure of the specified set of states in this NFA
   */
  private StateSet closure (final int startState)
  {

    // Out.debug("Calculating closure of "+set);

    final StateSet notvisited = tempStateSet;
    final StateSet closure = new StateSet (numStates, startState);
    ...
    while (notvisited.containsElements ())
    {
      ...
      final int state = notvisited.getAndRemoveElement ();
   ...
   */
  private StateSet closure (final StateSet startStates)
  {
    final StateSet result = new StateSet (numStates);
   ...
   * <code>start</code> with an specified input character <code>input</code>
   *
   * @param start
   ...
   *         <code>input</code>
   */
  private StateSet DFAEdge (final StateSet start, final int input)
  {
    // Out.debug(String.format("Calculating DFAEdge for state set "+start+" and
    // input U+04X"), input);
    ...
    final StateSet result = new StateSet (tempStateSet);
  ...
  public DFA getDFA ()
  {
    ...
    final Map <StateSet, Integer> dfaStates = new HashMap<> (numStates);
    final List <StateSet> dfaList = new ArrayList<> (numStates);

    final DFA dfa = new DFA (numEntryStates (), numInput, numLexStates);
    ...
    final StateSet tempStateSet = NFA.tempStateSet;
    final StateSetEnumerator states = NFA.states;
    ...
    while (currentDFAState <= numDFAStates)
    {
      ...
      for (int input = 0; input < numInput; input++)
      {
        ...
        // Out.debug("Calculating DFAEdge for state set "+currentState+" and
        // input '"+input+"'");
        ...
        if (newState.containsElements ())
        {
          ...
          final Integer nextDFAState = dfaStates.get (newState);
          ...
          else
          {
            ...
            final StateSet storeState = new StateSet (newState);
  ...
  public String toString ()
  {
    ...
    for (int i = 0; i < numStates; i++)
    {
      ...
      {
        result.append ("[FINAL");
        final String l = action[i].lookString ();
        if (!l.equals (""))
        {
          result.append (", ");
          result.append (l);
      ...
      for (int input = 0; input < numInput; input++)
      {
        if (table[i][input] != null && table[i][input].containsElements ())
          result.append ("  with ").append (input).append (" in ").append (table[i][input]).append (Out.NL);
  ...
  public void writeDot (final File file)
  {
    try
    {
  ...
  public String dotFormat ()
  {
    ...
    for (int i = 0; i < numStates; i++)
    {
      for (int input = 0; input < numInput; input++)
      {
        ...
        {
          final StateSetEnumerator states = table[i][input].states ();

          while (states.hasMoreElements ())
          {
            final int s = states.nextElement ();
      ...
      if (epsilon[i] != null)
      {
        final StateSetEnumerator states = epsilon[i].states ();
        while (states.hasMoreElements ())
        {
          final int s = states.nextElement ();
  ...
  private void insertLetterNFA (final boolean caseless, final int ch, final int start, final int end)
  {
    if (caseless)
    {
      final IntCharSet set = new IntCharSet (ch);
      final IntCharSet caselessSet = set.getCaseless (scanner.getUnicodeProperties ());
      for (final Interval interval : caselessSet.getIntervals ())
      {
  ...
  private IntPair insertStringNFA (final boolean caseless, final String str)
  {
    final int start = numStates;
    int i = 0;
    for (int pos = 0; pos < str.length (); ++i)
    {
      final int ch = str.codePointAt (pos);
      if (caseless)
      {
        final IntCharSet set = new IntCharSet (ch);
        final IntCharSet caselessSet = set.getCaseless (scanner.getUnicodeProperties ());
  ...
  private void insertClassNFA (final List <Interval> intervals, final int start, final int end)
  {
    // empty char class is ok:
    if (intervals == null)
      return;

    for (final int aCl : classes.getClassCodes (intervals))
      addTransition (start, aCl, end);
  }

  private void insertNotClassNFA (final List <Interval> intervals, final int start, final int end)
  {

    for (final int input : classes.getNotClassCodes (intervals))
   ...
   *         complement NFA.
   */
  private IntPair complement (final IntPair nfa)
  {
    ...
    final int start = dfaStart + numDFAStates + 1;
    final int error = dfaStart + numDFAStates + 2;
    final int end = dfaStart + numDFAStates + 3;
  ...
  private void removeDead (final int start, final int end)
  {
    ...
    for (int i = 0; i < numInput; i++)
    {
      final StateSet nextState = closure (table[start][i]);
      final StateSetEnumerator states = nextState.states ();
      while (states.hasMoreElements ())
      {
        final int next = states.nextElement ();
    ...
    final StateSet nextState = closure (epsilon[start]);
    final StateSetEnumerator states = nextState.states ();
    while (states.hasMoreElements ())
    {
      final int next = states.nextElement ();
   ...
   * exactly one start state, exactly one end state, no transitions leading out
   * of the end state no transitions leading into the start state Assumes that
   ...
   *         NFA.
   */
  private void insertCCLNFA (final RegExp regExp, final int start, final int end)
  {
    switch (regExp.type)
    {

      case sym.BAR:
        final RegExp2 r = (RegExp2) regExp;
  ...
  /**
   * Constructs an NFA for regExp such that the NFA has exactly one start state,
   * exactly one end state, no transitions leading out of the end state no
   * transitions leading into the start state
   * 
   * @param regExp
   *        the regular expression to construct the NFA for
   * @return a pair of integers denoting the index of start and end state of the
   *         NFA.
   */
  public IntPair insertNFA (final RegExp regExp)
  {
