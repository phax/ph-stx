 ...
 * filling in skeleton sections etc. Table compression, String packing etc. is
 * also done here.
 *
 * @author Gerwin Klein
 * @version JFlex 1.6.1
...
public final class Emitter
{
  ...
  public Emitter (final File inputFile, final LexParse parser, final DFA dfa) throws IOException
  {

    final String name = getBaseName (parser.scanner.className) + ".java";

    final File outputFile = normalize (name, inputFile);
   ...
   * @return the
   */
  public static String getBaseName (final String className)
  {
    final int gen = className.indexOf ('<');
    if (gen < 0)
    {
      return className;
    }
   ...
   * @param input
   *        fall back location if path = <tt>null</tt> (expected to be a file in
   *        the directory to write to)
   * @return The constructed File
   */
  public static File normalize (final String name, final File input)
  {
    File outputFile;
    ...
    {
      final File backup = new File (outputFile.toString () + "~");
  ...
  private void print (final int i, int tab)
  {
    int exp;
  ...
  private void emitMain ()
  {
    ...
    final String className = getBaseName (scanner.className);
  ...
  /**
   * Try to find out if user code ends with a javadoc comment, maybe followed by
   ...
   */
  public static boolean endsWithJavadoc (final StringBuilder usercode)
  {
    final Matcher matcher = JAVADOC_COMMENT_AND_MAYBE_ANNOTATIONS_PATTERN.matcher (usercode);
    return matcher.matches () && !matcher.group (1).contains ("*/");
  }

  private void emitLexicalStates ()
  {
    for (final String name : scanner.states.names ())
    {
      final int num = scanner.states.getNumber (name);
  ...
  private void emitDynamicInit ()
  {
    ...
    final CountEmitter e = new CountEmitter ("Trans");
  ...
  private void emitCharMapInitFunction (final int packedCharMapPairs)
  {

    final CharClasses cl = parser.getCharClasses ();
  ...
  private void emitCharMapArrayUnPacked ()
  {
    ...
    final int max = cl.getMaxCharCode ();
  ...
  /**
   * Returns the number of elements in the packed char map array, or zero if the
   * char map array will be not be packed. This will be more than
   * intervals.length if the count for any of the values is more than 0xFFFF,
   * since the number of char map array entries per value is ceil(count /
   * 0xFFFF)
   */
   ...
   * @prec 0 <= c <= 0xFFFF
   */
  private void printUC (final int c)
  {
    if (c > 255)
    {
      out.print ("\\u");
  ...
  private void emitRowMapArray ()
  {
    ...
    final HiLowEmitter e = new HiLowEmitter ("RowMap");
    e.emitInit ();
    for (int i = 0; i < dfa.numStates; i++)
    {
  ...
  private void emitAttributes ()
  {
    ...
    final CountEmitter e = new CountEmitter ("Attribute");
    e.emitInit ();
  ...
  private void emitConstructorDecl (final boolean printCtorArgs)
  {
    println ("  /**");
    ...
    final String warn = "// WARNING: this is a default constructor for " +
   ...
   */
  private String escapify (final String s)
  {
  ...
  public void emitActionTable ()
  {
    ...
    println ("   */");
    final CountEmitter e = new CountEmitter ("Action");
    e.emitInit ();
    ...
    for (int i = 0; i < dfa.numStates; i++)
    {
      ...
      if (dfa.isFinal[i])
      {
        final Action action = dfa.action[i];
  ...
  private void emitActions ()
  {
    ...
    for (final Map.Entry <Action, Integer> entry : actionTable.entrySet ())
    {
      ...
      if (action.lookAhead () == Action.FIXED_BASE)
      {
        ...
        println ("                (zzBufferL, zzStartRead, zzEndRead - zzStartRead, zzStartRead, " +
      ...
      if (action.lookAhead () == Action.FIXED_LOOK || action.lookAhead () == Action.FINITE_CHOICE)
      {
        ...
        println ("            zzMarkedPos = Character.offsetByCodePoints");
        println ("                (zzBufferL, zzStartRead, zzEndRead - zzStartRead, zzMarkedPos, -" +
                 action.getLookLength () +
                 ");");
      }
  ...
  private void emitEOFVal ()
  {
    ...
    if (eofActions.numActions () > 0)
    {
      ...
      {
        final int num = scanner.states.getNumber (name);
        final Action action = eofActions.getAction (num);
    ...
    final Action defaultAction = eofActions.getDefault ();
  ...
  public void emit ()
  {
    ...
    final int packedCharMapPairs = emitCharMapArray ();
