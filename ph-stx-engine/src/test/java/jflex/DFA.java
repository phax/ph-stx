...
public final class DFA
{
   ...
   * <code>current_state</code> with input <code>character</code>,
   * <code>NO_TARGET</code> if there is no transition for this input in
   * <code>current_state</code>
   */
  int [] [] table;

  /**
   * <code>isFinal[state] == true</code> <=> the state <code>state</code> is a
   * final state.
   */
  boolean [] isFinal;

  /**
   * <code>action[state]</code> is the action that is to be carried out in state
   * <code>state</code>, <code>null</code> if there is no action.
   */
  Action [] action;
  ...
  public DFA (final int numEntryStates, final int numInp, final int numLexStates)
  {
    numInput = numInp;

    final int statesNeeded = Math.max (numEntryStates, STATES);
  ...
  public void setEntryState (final int eState, final int trueState)
  {
    entryState[eState] = trueState;
  }

  private void ensureStateCapacity (final int newNumStates)
  {
    final int oldLength = isFinal.length;
    ...
    final boolean [] newFinal = new boolean [newLength];
    final boolean [] newPushback = new boolean [newLength];
    final Action [] newAction = new Action [newLength];
    final int [] [] newTable = new int [newLength] [numInput];
  ...
  public void setAction (final int state, final Action stateAction)
  {
  ...
  public void setFinal (final int state, final boolean isFinalState)
  {
    ...
    for (int i = 0; i < numStates; i++)
    {
      for (int input = 0; input < numInput; input++)
      {
        if (table[i][input] >= 0)
        {
          ...
          // result.append("
          // [label=\"[").append(classes.toString(input)).append("]\"]\n");
        }
      }
    }
   ...
   */
  public void checkActions (final LexScan scanner, final LexParse parser)
  {
    final EOFActions eofActions = parser.getEOFActions ();

    for (final Action a : scanner.actions)
   ...
   * description by D. Gries. Time: O(n log n) Space: O(c n), size < 4*(5*c*n +
   * 13*n + 3*c) byte
   */
  public void minimize ()
  {
    ...
    final int [] block = new int [2 * n];

    // implements a doubly linked list of states (these are the actual blocks)
    final int [] b_forward = new int [2 * n];
    final int [] b_backward = new int [2 * n];
    ...
    // (B_i, c) in L iff l_forward[(B_i-n)*numInput+c] > 0 // numeric value of
    // block 0 = n!
    final int [] l_forward = new int [n * numInput + 1];
    final int [] l_backward = new int [n * numInput + 1];
    final int anchorL = n * numInput; // list anchor

    // inverse of the transition table
    // if t = inv_delta[s][c] then { inv_delta_set[t], inv_delta_set[t+1], ..
    // inv_delta_set[k] }
    // is the set of states, with inv_delta_set[k] = -1 and inv_delta_set[j] >=
    // 0 for t <= j < k
    final int [] [] inv_delta = new int [n] [numInput];
    final int [] inv_delta_set = new int [2 * n * numInput];

    // twin stores two things:
    // twin[0]..twin[numSplit-1] is the list of blocks that have been split
    // twin[B_i] is the twin of block B_i
    final int [] twin = new int [2 * n];
    int numSplit;

    // SD[B_i] is the the number of states s in B_i with delta(s,a) in B_j
    // if SD[B_i] == block[B_i], there is no need to split
    final int [] SD = new int [2 * n]; // [only SD[n..2*n-1] is used]
    ...
    int lastDelta = 0;
    final int [] inv_lists = new int [n]; // holds a set of lists of states
    final int [] inv_list_last = new int [n]; // the last element
    for (int c = 0; c < numInput; c++)
    {
      ...
      // accumulate states of inverse delta into lists (inv_delta serves as head
      // of list)
      for (int s = 1; s < n; s++)
      {
        final int t = table[s - 1][c] + 1;
    ...
    // first, find the largest block B_max, then, all other (B_i,c) go into the
    // list
    int B_max = b0;
    int B_i;
    ...
    {
      final int last = l_backward[anchorL];
    ...
    while (l_forward[anchorL] != anchorL)
    {
      ...
      { // for each s in D
        ...
        // System.out.println("checking if block "+(B_i-b0)+" must be split
        // because of state "+s);
        ...
        if (SD[B_i] != block[B_i])
        {
          ...
          SD[B_i]--; // there is now one state less in B_i that goes with a into
                     // B_j
          // printBlocks(block, b_forward, b_backward, lastBlock);
      ...
      {
        B_i = twin[indexTwin];
        final int B_k = twin[B_i];
        for (int c = 0; c < numInput; c++)
        {
          final int B_i_c = (B_i - b0) * numInput + c;
          final int B_k_c = (B_k - b0) * numInput + c;
    ...
    /*
     * System.out.println("Old minimization:"); boolean [] [] equiv =
     * old_minimize(); boolean error = false; for (int i = 1; i < equiv.length;
     * i++) { for (int j = 0; j < equiv[i].length; j++) { if (equiv[i][j] !=
     * (block[i+1] == block[j+1])) {
     * System.out.println("error: equiv["+i+"]["+j+"] = "+equiv[i][j]+
     * ", block["+i+"] = "+block[i+1]+", block["+j+"] = "+block[j]); error =
     * true; } } } if (error) System.exit(1); System.out.println("check");
     */
  ...
  public void printBlocks (final int [] b, final int [] b_f, final int [] b_b, final int last)
  {
    ...
    for (int i = n; i <= last; i++)
    {
      ...
      while (s != i)
      {
        ...
        if (b_b[s] != t)
          Out.dump ("consistency error for b_back in state " +
                    (s - 1) +
                    " (back = " +
                    b_b[s] +
                    ", should be = " +
  ...
  public void printL (final int [] l_f, final int [] l_b, final int anchor)
  {
    String l = "L = {";
    int bc = l_f[anchor];
    while (bc != anchor)
    {
      final int b = bc / numInput;
      final int c = bc % numInput;
      l += "(" + b + "," + c + ")";
      final int old_bc = bc;
      bc = l_f[bc];
      if (bc != anchor)
  ...
  public boolean [] [] old_minimize ()
  {
    ...
    final boolean [] [] equiv = new boolean [numStates] [];

    // list[i][j] contains all pairs of states that have to be marked "not
    // equivalent"
    // if states i and j are recognized to be not equivalent
    final StatePairList [] [] list = new StatePairList [numStates] [];
    ...
    for (i = 1; i < numStates; i++)
    {
      ...
      for (j = 0; j < i; j++)
      {
        ...
        // i and j are both final and their actions are equivalent and have same
        // pushback behaviour or
  ...
  public void printInvDelta (final int [] [] inv_delta, final int [] inv_delta_set)
  {
  ...
  public void printTable (final boolean [] [] equiv)
  {
