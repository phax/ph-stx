...
/**
 * Abstract superclass for the JDK 1.4 and .NET regex translators, or in
 * principle for any other target regex dialect in which "." matches a UTF-16
 * 16-bit code rather than a Unicode character
 */
public abstract class SurrogateRegexTranslator extends RegexTranslator
{

  // TODO: could go further in terms of achieving reuse of common code between
  // the two subclasses.
  // The main thing needed is a factory method for the classes that differ, e.g.
  // Union and Subtraction,
  // which in turn means that static data using these classes needs to be
  ...
  protected static abstract class CharClass
  {
     ...
     *        none of the BMP characters
     * @param containsNonBmp
     *        NONE, SOME, or ALL, depending on whether the character class
     *        contains all the non-BMP characters, some of the non-BMP
     *        characters, or none of the non-BMP characters
     */
    ...
    public final void output (final FastStringBuffer buf)
    {
  ...
  /**
   * Simple Character Class - essentially, anything other than a Union or
   * Subtraction between two character classes.
   */

  public static abstract class SimpleCharClass extends CharClass
  {
    public SimpleCharClass (final int containsBmp, final int containsNonBmp)
    {
      super (containsBmp, containsNonBmp);
    }

    @Override
    public void outputBmp (final FastStringBuffer buf)
    {
      buf.append ('[');
    ...
    @Override
    public void outputComplementBmp (final FastStringBuffer buf)
    {
  ...
  public static class SingleChar extends SimpleCharClass
  {
    ...
    public SingleChar (final char c)
    {
      super (SOME, NONE);
      this.c = c;
    }
    ...
    @Override
    public void inClassOutputBmp (final FastStringBuffer buf)
    {
      if (isJavaMetaChar (c))
      {
        buf.append ('\\');
  ...
  public static class WideSingleChar extends SimpleCharClass
  {
    ...
    public WideSingleChar (final int c)
    {
      super (NONE, SOME);
      if (c <= 65535)
      {
  ...
  public static class CharRange extends SimpleCharClass
  {
    ...
    public CharRange (final int lower, final int upper)
    {
      super (lower < RegexData.NONBMP_MIN ? SOME : NONE,
             // don't use ALL here, because that requires that the BMP class
             // contains high surrogates
  ...
  public static class BackReference extends CharClass
  {
    ...
    public BackReference (final int i)
    {
      super (SOME, NONE);
      this.i = i;
    }
    ...
    void inClassOutputBmp (final FastStringBuffer buf)
    {
      if (i != -1)
      {
  ...
  /**
   * Character class representing the characters matched by the XPath "."
   * metacharacter
   */
  ...
  /**
   * Character class representing the complement of another character class,
   * that is, all characters that the other class doesn't match.
   */
  ...
  public static class Complement extends CharClass
  {
    ...
    public Complement (final CharClass cc)
    {
