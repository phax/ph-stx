 ...
 */
/*
 * $Id: CaseVariants.java,v 1.1 2007/06/04 19:57:37 obecker Exp $
 *
...
import net.sf.joost.util.om.IntHashMap;
import net.sf.joost.util.om.IntToIntHashMap;

/**
 * This class holds data about the case-variants of Unicode characters. The data
 * is automatically generated from the Unicode database.
 */
public class CaseVariants
{
  ...
  private static void cv (final int a, final int b)
  {
    monoVariants.put (a, b);
  }

  private static void cv (final int a, final int b, final int c)
  {
    final int [] v = { b, c };
    polyVariants.put (a, v);
  }

  private static void cv (final int a, final int b, final int c, final int d)
  {
    final int [] v = { b, c, d };
    polyVariants.put (a, v);
  }
  ...
  public static int [] getCaseVariants (final int code)
  {
    final int mono = monoVariants.get (code);
  ...
  /**
   * Get the case variants of roman letters (A-Z, a-z), other than the letters
   * A-Z and a-z themselves
   */

  public static int [] ROMAN_VARIANTS = { 0x0130, 0x0131, 0x212A, 0x017F };

  /**
   * The following data was generated from the Unicode database as follows: (a)
   * the database was converted to XML (b) the following query was run: let
   * $chars := doc('UnicodeData.xml')/ * /Char[string(toUpper) or
   * string(toLower)] for $c in $chars let $variants := ($chars[(code, toUpper)
   * = $c/(code, toUpper)] | $chars[(code, toLower) = $c/(code, toLower)])
   * except $c return if (count($variants) gt 0) then concat("cv(0x", $c/code,
   * ", 0x", string-join($variants/code, ", 0x"), ");&#xa;") else ()
   */
